...
issname = '셀바스AI'
isscode = kiwoomapi.getIssCode(issname)


class kiwoomapi_kiwoomapi(AppTester):
    ...
    def test01(self):
        ...
        for k in items:
            v = kiwoomapi.GetLoginInfo(k)
            print([k, v, type(v), len(v)])

        v = kiwoomapi.GetConnectState()
    ...
    def test04(self):
        ...
        v = kiwoomapi.SendCondition('9000', '0000', int('005'), 1)
        print(['SendCondition->', v, type(v)])
        v = kiwoomapi.SendCondition('9000', '0001', int('004'), 1)
        v = kiwoomapi.SendCondition('9000', '0002', int('001'), 1)
        v = kiwoomapi.SendCondition('9000', '0003', int('003'), 1)
        print(['SendCondition->', v, type(v)])

        # v = kiwoomapi.SendConditionStop('9000', '0000', int('005'))
    ...
    """Real데이타모니터링"""
    def test24(self):
        OpenAPI.SetRealReg(isscode)

    """#################### IssueAPI ####################"""
    def test101(self):
        o = kiwoomapi.IssueAPI(issname)
        self.set_object(o)
        o.set_sellcallprc02('4%')

        api = kiwoomapi.TrAPI('계좌평가잔고내역요청')
        self.set_object(api)
        api.req()
    """대량생성테스트"""
    def test102(self):
        ...
        self.trapi = kiwoomapi.TrAPI('계좌평가잔고내역요청')
        self.trapi.req()
    ...
    def test103(self):
        ...
        for id, val in tpls: kiwoomapi.SetInputValue(id, val)
        kiwoomapi.CommRqData(isscode, o.trcode, 0, o.screen_no)
    def test104(self):
        o = kiwoomapi.TrAPI('당일거래량상위요청')
        inputs = o.get()
        OpenAPI.SetTrReg(inputs, o.trname, o.trcode, 0, o.screen_no)
        for i in range(10):
            OpenAPI.SetTrReg(inputs, o.trname, o.trcode, 2, o.screen_no)
    def test105(self):
        o = kiwoomapi.TrAPI('시가대비등락률요청', maxLoop=1, 정렬구분='4')
        self.set_object(o)
        o.req()
    def test106(self):
        o = kiwoomapi.TrAPI('예상체결등락률상위요청', maxLoop=1)
        self.set_object(o)
        o.req()
    def test107(self):
        o = kiwoomapi.TrAPI('주식일봉차트조회요청', maxLoop=1, 종목코드='179290')
        self.set_object(o)
        o.req()
    ...
    """extParam:0"""
    def test201(self):
        # o = kiwoomapi.TrAPI('주식기본정보요청')
        # self.set_object(o)
        # o.req()

        def __view__(): print(pd.DataFrame(o.cumData))
        o = kiwoomapi.TrAPI('계좌평가잔고내역요청', maxLoop=None, timeout=10)
        self.set_object(o)
    ...
    """extParam:1"""
    def test202(self):
        # o = kiwoomapi.TrAPI('체결정보요청', 종목코드=isscode)
        # self.set_object(o)
        # o.req()
        # sleep(1)
        # o = kiwoomapi.TrAPI('주식분봉차트조회요청', 종목코드=isscode)
        # self.set_object(o)
        # o.req()
        # sleep(1)
        o = kiwoomapi.TrAPI('일별주가요청',종목코드=isscode)
        self.set_object(o)
        o.req()
    def test203(self):
        o = kiwoomapi.TrAPI('테마그룹별요청', 종목코드='', 검색구분='2')
        o.req()
    def test204(self):
        # P10101 -> 30
        # P10102 -> 30
        # '' -> 50
        o = kiwoomapi.TrAPI('투자자별일별매매종목요청', 시장구분='102')
        o.req()
        self.set_object(o)
    def test205(self):
        trname = '투자자별일별매매종목요청'
        o = kiwoomapi.TrAPI(trname)
        o.req()
        self.set_object(o)

        o = kiwoomapi.TrAPI(trname, 시장구분='001')
        o.req()
        self.set_object(o)

        o = kiwoomapi.TrAPI(trname, 시장구분='101')
        o.req()
        self.set_object(o)
    def test206(self):
        ...
        """
        o = kiwoomapi.TrAPI(trname, 시장구분='')
        o.req()
        self.set_object(o)
        sleep(2)
        o = kiwoomapi.TrAPI(trname, 시장구분='P00101')
        o.req()
        self.set_object(o)
        sleep(2)
        o = kiwoomapi.TrAPI(trname, 시장구분='P00102')
        o.req()
        self.set_object(o)
    def test207(self):
        trname = '프로그램매매추이차트요청'
        ...
        # 입력값 에러케이스
        # o = kiwoomapi.TrAPI(trname, 종목코드='')
        # o.req()
        # self.set_object(o)
        # sleep(2)

        # 둘다 같은 데이타 케이스
        o = kiwoomapi.TrAPI(trname, 종목코드='P10101')
        o.req()
        self.set_object(o)
        sleep(2)
        o = kiwoomapi.TrAPI(trname, 종목코드='P10102')
        o.req()
        self.set_object(o)
    def test208(self):
        """
    ...
    """신규매수::성공"""
    def test301(self):
        o = kiwoomapi.OrderAPI(issname,'신규매수','지정가',23900,1, timeout=2)
        self.set_object(o)
    ...
    """신규매수::실패"""
    def test302(self):
        o = kiwoomapi.OrderAPI(issname,'신규매수','지정가',23000,1, timeout=2)
        self.set_object(o)
    ...
    """신규매도::성공"""
    def test303(self):
        o = kiwoomapi.OrderAPI(issname,'신규매도','지정가',23000,1, timeout=10)
        self.set_object(o)
    ...
    """신규매도::실패"""
    def test304(self):
        o = kiwoomapi.OrderAPI(issname,'신규매도','지정가',24000,1, timeout=2)
        self.set_object(o)
    ...
    """신규매수::오류"""
    def test305(self):
        o = kiwoomapi.OrderAPI(issname,'신규매수','지정가',23000,1, timeout=10)
        self.set_object(o)
    ...
    """#################### MyFunctionalAPIs ####################"""
    def test401(self):
        v = kiwoomapi.getIssueInfo('121850')
        pp.pprint(v)
        if not isinstance(v, dict): raise

        v = kiwoomapi.isMoiServer()
        print(['isMoiServer', v, type(v)])
        if not isinstance(v, bool): raise
    def test402(self):

        v = kiwoomapi.get_cash()
        print(['get_cash', v, type(v)])
        if not isinstance(v, int): raise

    """#################### BackendServer ####################"""
    def test501(self): self.run_worker(kiwoomapi.BackendServer)
    def test502(self): self.run_worker(kiwoomapi.RawDataServer)
    def test503(self): self.run_worker(kiwoomapi.CondDataServer)
...
MainTester = kiwoomapi_kiwoomapi()
MainTester.run()
